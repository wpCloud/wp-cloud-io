...
function elastic_request( $url, $body = null, $args = null ) {
  global $current_site, $current_blog;

  if ( (! defined('WP_OBJECT_CACHE')) || (WP_OBJECT_CACHE === false) ) {
    return;
  }
  ...
  try {
    ...
    $url = 'http://wpcloud.io:9200/' . $current_blog->domain . '/'. $url;
    ...
    $_config[ CURLOPT_TIMEOUT ] = 5;
    $_config[ CURLOPT_CONNECTTIMEOUT ] = 5;

    if( $body ) {

      $args = (array) is_array( $args ) ? $args : array();

      $_config[ CURLOPT_CUSTOMREQUEST] = 'PUT';

      $_config[ CURLOPT_POSTFIELDS ] = json_encode(array_filter(array_merge( array(
        '_ttl' => null,
...
// send_request( '/_search' );
// send_request( '/test/askldj', array( "adsf" => "adfasf" ));
// send_request( '/test/askldj' );
...
class WP_Object_Cache {
  ...
  public function __construct( $persistent_id = NULL ) {
    ...
    if( defined( 'WP_MEMCACHED_SERVERS' ) && ( !isset( $memcached_servers ) || !$memcached_servers ) ) {
      $memcached_servers = array();

      foreach( explode( ',', WP_MEMCACHED_SERVERS ) as $_singleServer ) {
        $_singleServer = explode( ':', $_singleServer );
        $memcached_servers[] = array( $_singleServer[0], $_singleServer[1] );
      }
    }
...
//        die( '<pre>' . print_r( $this->m->getStats(), true ) . '</pre>' );
  ...
  public function add( $key, $value, $group = 'default', $expiration = 0, $server_key = '', $byKey = false ) {
    ...
    // the following lines are commented out for now since it causes AJAX calls to take forever
    //elastic_request( $group . '/' . $key, $value, array("_ttl" => $expiration));
  ...
  public function append( $key, $value, $group = 'default', $server_key = '', $byKey = false ) {
    ...
    // the following line is commented out for now since it causes AJAX calls to take forever
    //elastic_request( $group . '/' . $key, $value );
