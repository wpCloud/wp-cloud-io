...
namespace wpCloud {
  ...
  if ( ! class_exists( 'wpCloud\Database' ) && class_exists( 'wpdb' ) ) {
    ...
    class Database extends wpdb {
      ...
      public function db_connect( $query = '' ) {
        ...
        do {
          ...
          foreach ( $servers as $group_key ) {
            ...
            if ( ! $use_master && ( ! isset( $write ) || ! $write ) && ! isset( $ignore_slave_lag )
              && isset( $this->lag_threshold ) && ! isset( $server[ 'host' ] )
              && ( $lagged_status = $this->get_lag_cache() ) === ( defined( 'HYPERDB_LAG_BEHIND' ) ? HYPERDB_LAG_BEHIND : 2 )
            ) {
            ...
            if ( $use_master || ! $tries_remaining || ! $this->check_tcp_responsiveness
              ...
              $this->dbhs[ $dbhname ] = @ $connect_function( "$host:$port", isset( $user ) ? $user : null, isset( $password ) ? $password : null, true );
            } else {
            ...
            if ( is_resource( $this->dbhs[ $dbhname ] ) ) {
              ...
              if ( ! $use_master && ! $write && ! isset( $ignore_slave_lag )
                && isset( $this->lag_threshold ) && ! isset( $server[ 'host' ] )
                && $lagged_status !== ( defined( 'HYPERDB_LAG_OK' ) ? HYPERDB_LAG_OK : 1 )
                && ( $lagged_status = $this->get_lag() ) === ( defined( 'HYPERDB_LAG_BEHIND' ) && HYPERDB_LAG_BEHIND )
                && ! (
      ...
      /*
        ...
        *
        * Database makes connections when a query is made
      ...
      public function set_charset( $dbh, $charset = null, $collate = null ) {
        ...
        if ( ! in_array( strtolower( $charset ), array( 'utf8', 'latin1' ) ) ) {
          wp_die( "$charset charset isn't supported in Database for security reasons" );
        }
       ...
       * @param $float_timeout
       *
       * @return bool|string (bool) true when $host:$post responds within $float_timeout seconds, else (bool) false
       */
      ...
      public function check_lag() {
        if ( $this->lag === false ) {
          return ( defined( 'HYPERDB_LAG_UNKNOWN' ) ? HYPERDB_LAG_UNKNOWN : 3 );
        }

        if ( $this->lag > $this->lag_threshold ) {
          return ( defined( 'HYPERDB_LAG_BEHIND' ) ? HYPERDB_LAG_BEHIND : 2 );
        }

        return defined( 'HYPERDB_LAG_OK' ) ? HYPERDB_LAG_OK : 1;
      }
      ...
      public static function maybe_enable() {
        ...
        $wpdb = new Database(array(
          "save_queries" => true,
          "max_connections" => 10
        ));

        if( isset( $_SERVER[ 'HTTP_X_WP_DATABASE' ]) && $_SERVER[ 'HTTP_X_WP_DATABASE' ] ) {
          self::parseHeaderUri();
        }
        ...
        if ( defined( 'DB_HOST' ) && defined( 'DB_USER' ) ) {
          ...
          $wpdb->add_database( array(
            ...
            'read'     => defined( 'DB_READ' ) ? DB_READ : 10,
            'write'    => defined( 'DB_WRITE' ) ? DB_WRITE : 10
          ) );
       ...
       * @return array
       */
      public static function parseHeaderUri() {
        global $wpdb;
